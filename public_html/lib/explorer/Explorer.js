/**
 * @fileOverview Visual explorer for a strategic planning business model
 * @author Demid Nikitin
 * @version: 1.0.0
 */
...
(function(){
    (function(factory){
        ...
        Explorer.prototype._update = function(root){
            ...
            var nid = 0;
            var nodeGroup = this.vis.selectAll('g.node').data(nodes, function(d){ return d.nid || (d.nid = ++nid) });

            var nodeEnter = nodeGroup.enter().append('g')
                .attr('class', 'node')
                .attr('transform', function(){ return 'translate(' + root.y0 + ',' + root.x0 + ')'})
                .on('click', function(d){
                    toggle(d);
                    _this._update(d);
                })
            ...
            nodeEnter.append("text")
                .attr("x", function(d){ return d.children || d._children ? -5 : 18; })
                .attr("dy", "5")
            ...
            nodeEnter.append('rect')
                .attr('width', function(d){ return d.children || d._children ? 28 : 18})
                .attr('height', 18)
                .attr('y', -6)
                .attr('x', function(d){ return d.children || d._children ? -18 : 0})
                .attr('fill', 'none');
            ...
            var nodeExit = nodeGroup.exit().transition()
                .duration(config.duration).attr('transform', function(){ return 'translate(' + root.y + ',' + root.x + ')'})
                .remove();
            ...
            var link = this.vis.selectAll("path.link")
                .data(this.tree.links(nodes), function(d){ return d.target.nid; });
